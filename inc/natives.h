...
extern Player CONVERT_INT_TO_PLAYERINDEX(uint playerId); //online player menu
...
extern void GET_PLAYER_CHAR(Player playerIndex, Ped *pPed); //online player menu
...
extern void GET_CHAR_VELOCITY(Ped ped, float *pX, float *pY, float *pZ);//super jump and what not?
...
extern void SET_CHAR_VELOCITY(Ped ped, float x, float y, float z); //super jump?
...
extern void TASK_ENTER_CAR_AS_PASSENGER(Ped ped, Vehicle vehicle, uint duration, uint seatIndex);//random peds in the car
...
extern void CREATE_CHAR_AS_PASSENGER(Vehicle vehicle, uint charType, uint model, uint passengerIndex, Ped *pPed);//lolxzssss
...
extern void CREATE_RANDOM_CHAR_AS_PASSENGER(Vehicle vehicle, uint seat, Ped *pPed);//random peds!!!!!!!
...
extern uint GET_PED_MODEL_FROM_INDEX(int index); //online ped menu
...
extern void SET_OBJECT_AS_STEALABLE(Object obj, boolean set); //THROWABLE BOWLING BALLS WOOOOOOO
...
extern void TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME(char *name); //get rid of phone
...
void GET_PED_FROM_NETWORK_ID(int id, Ped *ped); //code word poopies
...
void GET_NETWORK_ID_FROM_PED(ScriptAny, ScriptAny); //poop 2
...
ScriptAny DOES_PED_EXIST_WITH_NETWORK_ID(ScriptAny);//poop 3
